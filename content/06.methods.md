...
#### Breakpoint Density (`chromosomal-instability` analysis module)
...
For consensus CNV calls, if |log<sub>2</sub> ratio| > log<sub>2</sub>(1 + 0.5), we annotated the segment as a break. 
We then calculated breakpoint density as:

$$\textrm{breakpoint density} = \frac{\textrm{N breaks x 1,000,000}}{\textrm{Size in bp of }\textit{effectively surveyed}\textrm{ genome or exome}}$$
