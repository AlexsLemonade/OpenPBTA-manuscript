...
NantHealth (Culver City, CA) performed whole genome sequencing (WGS) on all paired tumor (~60X) and constitutive (~30X) DNA samples using 100 bp libraries.
NantHealth performed whole transcriptome stranded RNA-Seq to an average depth of 100M reads for CBTTC tumor samples.
