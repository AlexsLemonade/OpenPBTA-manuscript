...
Specifically, we implemented strategies to maintain a consistent software development environment that ensured code could be revised without inducing conflicts.
We leveraged Docker® and the Rocker project [@arxiv:1710.03675] to create a monolithic image that contained all dependencies necessary for analyses.
...
By contrast, the classifier performed poorly on poly-A samples
While this classifier has previously shown strong performance on poly-A data from adult tumors [@doi:10.1016/j.celrep.2019.09.071], it did not perform as well on the poly-A samples in this cohort (AUROC = 0.62; **Figure {@fig:S5}A**).
