...
A summarized view of the number of biospecimens per phase of therapy across different histology groupings is shown in (**Figure {@fig:Fig1}B**). 
We maintained a data release folder on Amazon S3, downloadable using the GitHub repository or the open-access [CAVATICA project](https://www.cavatica.org/), containing merged files for each analysis (See **Data and code availability** section). **TODO: this is mentioned before mention the repo itself!**
...
A key innovative feature of this project was its open contribution model used for both analyses (i.e., analytical code) and scientific manuscript writing. 
...
First, a potential contributor would proposed an analysis by filing an issue in the GitHub analysis repository. 
...
Because the CBTN opened its protocol before molecular subtypes were integrated into classifications, the majority of the samples within the OpenPBTA lacked molecular subtype annotations. 
Moreover, the OpenPBTA data does not feature methylation arrays which are commonly used to perform molecular subtyping.
...
Importantly, this collaborative molecular subtyping process allowed us to identify data entry errors (e.g., an ETMR entered as a medulloblastoma) and mis-identified specimens (e.g., Ewing sarcoma sample labeled as a craniopharyngioma) (**TODO: how is it actually possible to tell the difference between mislabeled and misspecified?**), update diagnoses using modern terms (e.g., primitive neuro-ectodermal tumor [PNET] diagnoses **TODO: IS THIS MODERN OR NOT? What is if updated to/from?**), and discover rarer tumor entities within the OpenPBTA (e.g., H3-mutant ependymoma, meningioma with _YAP1::FAM118B_ fusion).
...
In **Figure {@fig:Fig2}C**, we show genomic alterations in diffuse midline gliomas (DMGs, N = 34) and non-midline high-grade gliomas (HGGs, N = 26) biopsied at diagnosis. 
...
Gene set enrichment analysis showed upregulation (ANOVA p < 0.01) of DNA repair, G2M checkpoint, and MYC pathways as well as downregulation of the TP53 pathway (**Figure {@fig:Fig5}B**). **TODO: if we want to reference a figure 5 panel _here_, we need to moe the panel. Otherwise figures are out of order.**
...
Although LGG and embryonal tumors made up the majority of tumor types within the OpenPBTA, most of the significant gene interactions stemmed from HGGs (N = 847/872). **TODO: Is this denominator the total number of significant? Unclear what these numbers are**
...
We observed that 52/54 (96.3%) of tumors only have a mutation in either gene (OR = 0.188, 95% CI: 0.04 - 0.94, p = 4.13e-2, q = 5.87e-2 **TODO THIS IS NOT SIGNIFICANT??**), recapitulating a previously observed trend that _TP53_ and _PPM1D_ mutations tend to be mutually exclusive in HGGs [@doi:10.1038/ng.2938].
...
We identified chromothripsis events in 41% (N = 19/46) of non-midline high-grade gliomas and in 28.2% (N = 11/39) of DMGs (**Figure {@fig:Fig3}D**). 
We also found evidence of chromothripsis in over 15% of embryonal tumors, ependymomas, meningiomas, germinomas, glial-neuronal tumors, chordomas, metastatic secondary tumors, and sarcomas, highlighting the genomic instability and complexity of pediatric brain tumors. **TODO: 15% of EACH of these groups, or TOTAL 15%?**

We next assessed the contributions of eight previously identified adult CNS-specific mutational signatures from the RegSig database [@doi:10.1038/s43018-020-0027-5] across samples (**Figure {@fig:Fig3}E** and **Figure {@fig:S4}A**).
