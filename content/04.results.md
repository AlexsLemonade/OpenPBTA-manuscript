...
HGATs, followed by medulloblastomas, have the most genomically unstable genomes compared to copy number silent craniopharyngiomas (**Figure S3A**), consistent with their respective median TMBs and distribution (**Figure S2G**). 
The number of SV and CNV breakpoints were significantly correlated across tumors (Adjusted R = 0.436, p = 1.08e-37) (**Figure {@fig:Fig3}D**) and as expected, as breakpoint density increases, the number of chromothripsis regions called increases (**Figure S3B-C**).
...
We performed UMAP (Uniform Manifold Approximation and Projection) unsupervised dimension reduction to validate that brain tumor histologies cluster based on tissue of origin (**Figure {@fig:Fig4}A**) and geneset variant analysis (GSVA) for Hallmark cancer gene sets to demonstrate activation of underlying oncogenic pathways (**Figure {@fig:Fig4}B**).	
We observed medulloblastoma samples cluster by molecular subtype with Groups 3 and 4 tightly clustered and WNT and SHH in distinct clusters (**Figure {@fig:FigS4}A**), as expected.
_C11orf95::RELA_ fusion positive ependymomas clustered together and _BRAF_-driven low-grade tumors cluster 
We next used gene expression data to predict telomerase activity using EXpression-based Telomerase ENzymatic activity Detection (EXTEND) [@doi:10.1038/s41467-020-20474-9].
EXTEND scores calculated using FPKM or counts were tightly correlated (Pearson's R >= 0.94, p < 2.2e-16) for each library type (**Figure {@fig:FigS4}A-B**) and significantly correlated with _TERT_ (Pearson's R >= 0.55) and _TERC_ expression (Pearson's R >= 0.55)(**Figure {@fig:FigS4}C-F**).  
