...
**Figure {@fig:Fig2}** and **Figure {@fig:S3}B** depict oncoprints recapitulating known histology-specific driver genes in primary tumors across PBTA histologies.
...
The breakpoint density estimated from SV and CNV data was significantly correlated across tumors (linear regression p = 1.08e-37) (**Figure {@fig:Fig3}C**) and as expected, the number of chromothripsis regions called increased as breakpoint density increased (**Figure {@fig:S3}C-D**).
...
Although we observed low _TP53_ scores in two tumors from LFS patients (BS_DEHJF4C7 with a score of 0.09 and BS_ZD5HN296 with a score of 0.28), we confirmed from pathology reports that both diagnosed with LFS and had a pathogenic germline variant in _TP53_.
...
While we did not find that tumors with _TERT_ promoter (TERTp) mutations (N = 6) had significantly higher telomerase activity scores than non-mutated tumors (W = 1840, p-value = 0.1196), we observed `EXTEND` scores significantly correlated with _TERC_ (R = 0.619, p < 0.01) and _TERT_ (R = 0.491, p < 0.01) expression (**Figure {@fig:S5}B-C**).
We likely have too few samples to detect a significant effect of TERTp on telomerase activity and active telomerase requires a combination of full-length _TERT_, _TERC_, as well as accessory proteins [@PMID: 9751630]. 
