...
We performed UMAP unsupervised dimension reduction to validate that brain tumor histologies cluster based on tissue of origin (**Figure {@fig:Fig4}A**) and geneset variant analysis (GSVA) for Hallmark cancer gene sets to demonstrate activation of underlying oncogenic pathways (**Figure {@fig:Fig4}B**).	
...
EXTEND scores calculated using FPKM or counts were tightly correlated (p < 2.2e-16) for each library type (**Figure {@fig:FigS4}A-B**) and significantly correlated with _TERT_ and _TERC_ expression (**Figure {@fig:FigS4}C-F**).  
