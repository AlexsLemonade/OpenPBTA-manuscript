...
We performed geneset variant analysis (GSVA) for Hallmark cancer gene sets, demonstrating activation of underlying oncogenic pathways (**Figure {@fig:Fig4}B**).	
...
To understand the _TP53_ phenotype in each tumor, we ran a classifier previously trained on TCGA gene expression data[@doi:10.1016/j.celrep.2018.03.076] to infer _TP53_ inactivation status.
Using high-confidence SNVs, CNVs, SVs, and fusions in _TP53_ as true positive alterations, we achieved a high accuracy (AUROC = 0.85) compared to randomly shuffled _TP53_ scores (**Figure {@fig:Fig5}A**) in the stranded RNA-Seq dataset. 
The classifier did not perform well on the poly-A sample subset (**Figure {@fig:FigS5}A-B**), likely due to the low number of poly-A samples in our dataset (N = 58), as we previously reported high accuracy of this classifier on another poly-A dataset [@doi:10.1016/j.celrep.2019.09.071].
...
Surprisingly, we observed that samples annotated as either "activated" or "loss" had significantly higher _TP53_ scores than those annotated as "other" (**Figure {@fig:Fig5}B**, p<sub>adj</sub>loss vs. other < 2e-16, p<sub>adj</sub> activated vs. other = 4.0e-5), suggesting that the classifier detects an oncogenic phenotype of _TP53_ (scores > 0.5) rather than solely _TP53_ inactivation.
Moreover, tumors with "activating" _TP53_ mutations had evidence of higher _TP53_ expression than those with _TP53_ "loss" mutations (Wilcoxon p = 3.5e-3, **Figure {@fig:Fig5}B**.
...
Of note, we were unable to detect canonical somatic _TP53_ mutations in two LFS patient tumors, annotated as "other" in **Figure {@fig:Fig5}C**.
We confirmed the LFS diagnosis from pathology reports and found each to have a germline variant in _TP53_, however, the tumor purity of these samples was low (16% and 37%), suggesting the classifier might require a certain level of tumor purity for high accuracy.
Additionally, we found a significant positive correlation between _TP53_ scores and CNV breakpoint density (**Figure {@fig:FigS5}C**), consistent with our previously report [@doi:10.1016/j.celrep.2019.09.071].
...
![**Figure 5. TP53 and ** A, B](images/main/Figure5.png){#fig:Fig5}
