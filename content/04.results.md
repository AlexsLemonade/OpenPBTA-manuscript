...
A) First two dimensions from Uniform Manifold Approximation and Projection (UMAP).
Points are colored by `short_histology`.
B) Heatmap of GSVA scores for Hallmark gene sets with significant differences, ordered by `short_histology`.
C) Heatmap of xCell fraction values for scores and cell types with high variance, ordered by `short_histology`.
