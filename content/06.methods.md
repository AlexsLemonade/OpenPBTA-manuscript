...
All original code was developed within the following repositories and is publicly available as followgs. 
...
Associated DOIs and software versions used for analyses are listed in the **Key Resources Table**.
...
All CBTN data can be downloaded from the Gabriella Miller Kids First Data Resource Center (See **Data and code availability** above). **TODO: I think this is redundant with the data statements above, but if not we should make sure the same name is used above and here, ie pick "Gabriella Miller Kids First Data Resource Center" or "KidsFirst Portal"**
...
Briefly, 10-20 mg frozen tissue, 0.4-1ml of blood, or 2e6 **TODO just a note I did this for consistency since main text uses `e`** cells pellet was used for extractions.
...
The NantHealth Sequencing Center, BGI at CHOP **TODO: Should this say BGI@CHOP?**, or the Genomic Clinical Core at Sidra Medical and Research Center performed library preparation and sequencing.
We prepared DNA sequencing libraries for tumor and matched-normal DNA using the KAPA HyperPrep kit (cat# 08098107702, Roche), and we prepared tumor RNA-Seq libraries using the KAPA Stranded RNA-Seq with RiboErase kit (cat# 07962304001, Roche).
...
BGI at CHOP **TODO: Should this say BGI@CHOP?** performed poly-A or ribosomal-depleted whole transcriptome stranded RNA-Seq to an average depth of 100M.
...
We required that at least 60% of RNA-Seq reads mapped to the human reference before samples were removed from **TODO: Should this sentence say _INCLUDED IN_ and not _removed from_?** analysis. 
...
We normalized INDELs with `bcftools norm` on all PASS VCFs using the `kfdrc_annot_vcf_sub_wf.cwl` subworkflow, release v3 (See **Key Resources Table**).
...
We annotated SNV and INDEL hotspots from v2 of Memorial Sloan Ketting Cancer Center's (MSKCC) database (See **Key Resources Table**) as well as the TERT promoter mutations C228T and C250T [@doi:10.3390/ijms21176034].
...
We removed non-hotspot annotated variants with a normal depth less than or equal to 7 and/or gnomAD allele frequeny (AF) greater than 0.001 as potential germline variants.
...
We removed sample and caller combination files with more than 2500 CNVs called because we expected these combinations to be noisy and derive poor quality samples based on cutoffs used in GISTIC [@doi:10.1186/gb-2011-12-4-r41]. 
...
We did not include any copy number alteration that was not called by two or more callers in the consensus file. 
...
We used [AnnotSV v2.1](https://lbgi.fr/AnnotSV/) [@doi:10.1093/bioinformatics/bty304] to annotate Manta output. **TODO: AnnotSV is not in the Key Resources file because it was not in the OpenPBTA-workflows repo README. Should that README also be updated then? We will need to confirm that we got everything here.**
This associated workflow is available in the [D3b GitHub repository](https://github.com/d3b-center/annotSV/tree/master). **TODO: Confused, which repo? This or the OpenPBTA-workflows?**

#### Gene Expression

##### Abundance Estimation

**TODO: From this section's TODOs, the workflow repo's README _definitely_ needs to be revisited a bit**
...
We used Ensembl GENCODE 27 "Comprehensive gene annotation" (see **Key Resources Table**) as a reference. **TODO: this is not in the key resources table.**
We used RSEM [@doi:10/cwg8n5] v1.3.1 for both FPKM and TPM transcript- and gene-level quantification. **TODO: this is not in the key resources table.**
In addition, we used kallisto [@doi:10.1038/nbt.3519] v0.43.1, which uses a pseudoalignment strategy, as a second method of quantification. **TODO: this is not in the key resources table. ALSO, kallisto isn't in the manuscript so do we need to mention kallisto at all?**
...
After collapsing gene identifiers, 46,400 uniquely-expressed genes remained in the poly-A dataset, and 53,011 uniquely-expressed genes remained in the stranded dataset. **TODO: Do we have a clear source for these numbers?**
...
We used [FusionAnnotator](https://github.com/FusionAnnotator/FusionAnnotator) on Arriba fusion calls to harmonize annotations with those of STAR-Fusion. **TODO: Missing from key resources and the workflow README**
...
##### Recurrently mutated genes and co-occurrence of gene mutations (`interation-plots` analysis module **TODO is this right?**)
...
We used the ENSEMBL Variant Effect Predictor [@doi:10/gdz75c] annotations **TODO: is this versioned?**, including "High" and "Moderate" consequence types as defined in the R package Maftools [@doi:10.1101/gr.239244.118], to determine the set of nonsynonymous mutations.
...
**TODO: should these sentences be here? The TCGA comes a bit out of nowhere.**
Lancet was biased towards calling mutations with a much lower VAF for WXS than for WGS data (**Figure S2G**).
...
**Table S1** contains a README, along with sample technical, clinical, and additional metadata used for this study. **TODO: I don't think we strictly need all of this text since it's all documented in the README. It should be enough to just reference Table S1, and these nitty-gritty details about individual column definitions aren't really needed to explain he subtyping**
...
We performed molecular subtyping tumors in the OpenPBTA with unknown subtypes, to the extent possible.
The `molecular_subtype` field in the `pbta-histologies.tsv` file contains molecular subtypes for tumor types selected from  `pathology_diagnosis` and `pathology_free_text_diagnosis` fields as described below, following World Health Organization 2016 classification criteria [@doi:10.1007/s00401-016-1545-1]. **TODO: Also not convinced we need this sentence since it's all in the README.**
...
If the tumor's primary_site of the tumor was Ventricles, we assigned the subtype as `CNC`; otherwise, we assigned the subtype as `EVN`.
...
The compilation was executed by the following steps: **TODO: I do not think this next level of detail is necessary for the manuscript?**
...
**TODO: We need an approach for these upcoming tables because they are currently not "real table."" We either need to make them real tables or scrap them.**
...
In addition to annotating molecular subtypes as described above, we additionally annotated samples as according to their TP53 altered status status, either `TP53 lost` or `TP53 activated` (`tp53_nf1_score` analysis module.)
This annotation was additionally included in the molecular subtyping information.
To this end, we applied a _TP53_ inactivation classifier originally trained on TCGA PanCan data [@doi:10.1016/j.celrep.2018.03.076] to the matched RNA expression data for each sample. 
Along with the _TP53_ classifier scores, we collectively used consensus SNV and CNV, SV, and reference databases that list TP53 hotspot mutations [@doi:10.1158/2159-8290.CD-17-0321; @doi:10.1038/nbt.3391] and functional domains [@doi:10.1038/sj.cdd.4401904] to determine TP53 alteration status for each sample.
We adopted the following rules for calling either `TP53 lost` or `TP53 activated`:

1. If a sample had any of the two well-characterized _TP53_ gain-of-function mutations, p.R273C or p.R248W [@doi:10.1038/ng0593-42], we assigned `TP53 activated` status.

2. Samples were annotated as `TP53 loss` if any of they contained i) a _TP53_ hotspot mutation as defined by IARC TP53 database or the MSKCC cancer hotspots database [@doi:10.1158/2159-8290.CD-17-0321; @doi:10.1038/nbt.3391] (see also, **Key Resources Table**), ii) two _TP53_ alterations, including SNV, CNV or SV, which would be indicative of probable bi-allelic alterations; iii) one _TP53_ somatic alteration, including SNV, CNV, or SV or a germline _TP53_ mutation indicated by the diagnosis of Li-Fraumeni syndrome [@doi:10.1101/cshperspect.a026187], or iv) one germline _TP53_ mutation indicated by Li-Fraumeni syndrome and the _TP53_ classifier score for matched RNA-Seq was greater than 0.5. 
...
We reviewed this statistic in the context of reported gender and determined that a threshold of less than 0.2 clearly delineated female samples. **TODO: How was this threshold determined? Manually?**
...
If this filtering still resulted in multiple specimens, we selected from the remaining set randomly (`independent-samples` analysis module).

#### Quantification of Telomerase Activity using Gene Expression Data

We predicted telomerase activity of tumor samples using the recently developed EXTEND method [@doi:https://doi.org/10.1101/2020.05.21.109249] (`telomerase-activity-prediction` analysis module).
...
**TODO Double check LGG/HGG situation.**
We calculated overall survival (OS) as days since initial diagnosis (`OS_days`) and progression-free survival (PFS) as days since last event (`PFS_days`) and performed several survival analyses on the OpenPBTA cohort using the [`survival` R package](https://cran.r-project.org/package=survival) (`survival-analysis` analysis module). **TODO: Can we get rid of PFS since it's not in the MS?**
...
**TODO this is probably old and should be deleted since our key resources table is a massive separate Excel now**
