...
We first annotated germline variants in genes included in the KEGG MMR gene set (see **Key Resources**), _POLE_, and _TP53_ using SnpEff [@doi:10.4161/fly.19695] and ANNOVAR [@doi:10.1093/nar/gkq603]. 
Variants with read-depth >= 15, variant allele fraction >= 0.20, and observed in < 0.1% across each population in the public control databases non-TCGA ExAC (exonic) or gnomAD 2.1 (non-exonic, splicing) were considered for further study. 
We assessed pathogenicity of retained variants _in silico_ using custom code to evaluate ClinVar and a modified execution of InterVar [@10.1016/j.ajhg.2017.01.004]. 
We considered ClinVar calls in a hierarchical manner: (1) expert panel decision, (2) consensus of “badged labs”, when available. 
Badged labs were defined as laboratories meeting minimum requirements for data sharing as defined by [ClinGen](https://www.clinicalgenome.org/tools/clinical-lab-data-sharing-list/). 
Next, prior to InterVar evaluation, we adjusted PP5 based on this modified ClinVar assessment and corrected PVS1 using AutoPVS140 [@doi:10.1002/humu.24051]. 
Variants were then assigned to be pathogenic (P), likely-pathogenic (LP), benign (B), likely-benign (LB) or variants of unknown significance (VUS) by assigning the adjusted ClinVar [@doi:10.1093/nar/gkt1113] results if available, or our modified InterVar assessment when no ClinVar call could be made. 
All P/LP calls were manually reviewed by an interdisciplinary team, including clinicians and genetic counselors. 
...
|ExAC variants (non-TCGA)                    |Exome Aggregation Consortium                        |ftp://ftp.broadinstitute.org/pub/ExAC_release/release0.3.1/subsets/                     |
|gnomAD v2.1.1                    |Genome Aggregation Database                        |https://gnomad.broadinstitute.org/downloads#v2-liftover-variants                     |
|KEGG MMR gene set                    |BROAD Institute                        |https://www.gsea-msigdb.org/gsea/msigdb/download_geneset.jsp?geneSetName=KEGG_MISMATCH_REPAIR&fileType=TSV                     |
|ClinVar Database, 20220507                   |NCBI          |https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/archive_2.0/2022/             |
|                    |                        |                     |
|                    |                        |                     |
|                    |                        |                     |