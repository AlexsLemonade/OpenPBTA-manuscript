...
We further investigated the mutational signature profiles of the hypermutant (N = 2) and ultra-hypermutant (N = 4) tumors and/or derived cell lines from five patients in the OpenPBTA cohort.
The relative contributions of eight RefSig mutational signatures are shown in the heatmap in **Figure {@fig:Fig4}E** for all tumors from patients with at least one tumor or cell line with a TMB >= 10 Mut/Mb.
Signature 11, a suggested exposure to temozolomide, was indeed present in tumors with previous exposure to the drug.
We found the MMR2 signature in tumors of four patients (PT_0SPKM4S8, PT_3CHB9PK5, PT_JNEV57VK, and PT_VTM2STE3) diagnosed with Lynch syndrome (**\+tbl:T2**), a genetic predisposition syndrome caused by a variant in a mismatch repair gene such as _PMS2_, _MLH1_, _MSH2_, _MSH6_, or others [@doi:10.3390/genes5030497].
Interestingly, the cell line derived from patient PT_VTM2STE3's tumor at progression exposed to _only_ MMR2, but was not hypermutated (TMB = 5.74), suggesting there was selective pressure to maintain an MMR phenotype _in vitro_.
From patient PT_JNEV57VK, only one of the two cell lines derived from a progressive tumor was hypermutant (TMB of 35.9 Mut/Mb).
The hypermutated cell line had a strong signature 11, while the non-hypermutated cell line from the same tumor showed more smaller exposures (1, 11, 18, 19, MMR2), highlighting the importance of carefully genomically characterizing and selecting models for preclinical studies.
...
All tumors and cell lines were HGGs that we found to have dysfunctional _TP53_ (**\+tbl:T2**), consistent with a previous report showing dysregulation of _TP53_ is a dependency in tumors with high genomic instability [@doi:10.1038/s43018-020-0027-5].
...
Next, we asked whether transcriptomic scoring of _TP53_ dysregulation and/or telomerase activity recapitulate the known prognostic influence of these oncogenic biomarkers.
We split the OpenPBTA cohort into HGG (HGGs or DMGs) and non-HGG tumors and performed multivariate cox regression for overall survival (**Figure {@fig:Fig4}F**).
As expected, we show a significant negative effect on survival if the tumor belonged to the HGG group (HR = 8.7, CI = 5.59-13.52, p < 0.01).
Additionally, high telomerase scores were poor prognostic indicator across brain tumor histologies (HR = 41.97, CI = 13.16-133.93, p < 0.01), demonstrating that EXTEND scores calculated from RNA-Seq can be used as a rapid surrogate measure for telomerase activity.
Higher _TP53_ scores, indicative of dysregulation of the _TP53_ gene or pathway, neared significance (HR = 2.25, CI = 0.93-5.47, p = 0.072) for an increased risk of lower overall survival.

Finally, for HGGs only, we assessed the effect of molecular subtypes with and without _TP53_ dysregulation, as determined by _TP53_ classification combined with molecular evidence.
Cox proportional hazards analysis (**Figure {@fig:Fig4}G**) recapitulate the known increased risk of the H3 K28 mutation compared to H3 wildtype (BH-adjusted p = 0.0027) [@doi:10.1093/neuonc/nox149].
Furthermore, using _TP53_ classification, we show that DMG H3 K28 tumors with _TP53_ loss have worse prognosis than DMG H3 K28 tumors with wildtype _TP53_.
Overall survival of HGGs by molecular subtype are visualized by Kaplan-meier in (**Figure {@fig:Fig4}H**). 

![**Figure 4. _TP53_ and telomerase activity** A, Receiver Operating Characteristic for _TP53_ classifier run on FPKM of stranded RNA-Seq samples. B, Violin and box plots of _TP53_ scores plotted by _TP53_ alteration type (N<sub>activated</sub> = 27, N<sub>lost</sub> = 140, N<sub>other</sub> = 999). C, Violin and box plots of _TP53_ RNA expression plotted by _TP53_ activation status (N<sub>activated</sub> = 11, N<sub>lost</sub> = 96, N<sub>other</sub> = 862). D, Box plots of _TP53_ and telomerase (EXTEND) scores grouped by `cancer_group`. Mutation status is highlighted in orange (hypermutant) or red (ultra-hypermutant). E, Heatmap of RefSig mutational signatures for patients who have least one tumor with a TMB >= 10 Mut/Mb. F, Forest plot depicting the prognostic effects of _TP53_ and telomerase scores on overall survival. G, Forest plot depicting the effect of molecular subtype on overall survival of HGGs. For F and G, hazard ratio (HR) with 95% confidence intervals and p-values are listed. Significant p-values are denoted with colored diamonds. H, Kaplan-meier curve of HGG tumors by molecular subtype.](images/main/Figure4.png){#fig:Fig4}
...
Although CD8+ T-cell infiltration across all cancer groups is quite low (**Figure {@fig:Fig5}C**), we observed some signal in specific cancer molecular subtypes (Groups 3 and 4 medulloblastoma) as well as outlier tumors (BRAF-driven LGG, BRAF-driven and wildtype ganglioglioma, H3 wildtype HGG).   
There is growing evidence of an inflammatory tumor immune microenvironment (TIME) in HGGs [@doi:10.1093/brain/awab155] and DMGs and tumors in these two histologies do in fact contain the highest overall proportions of immune cells, with monocytes, dendritic cells, and NK cells most prevalent.
...
Medulloblastomas typically have low expression of _CD274_ (PD-L1) [@doi:10.18632/oncotarget.24951] a result we reproduced here.
However, we also found that higher expression of _CD274_ is significantly associated with better overall prognosis (HR = 0.0019, CI = 1.7e-5 to 0.22, p = 0.009), suggesting these tumors may have a better response to PD-1 checkpoint blockade [@doi:10.1158/1078-0432.CCR-13-3271].
Consistent with a previous study, we found higher expression of _CD274_ in the WNT subgroup which has the best prognosis of all medulloblastoma subgroups [@doi:10.1080/2162402X.2018.1462430] (**Figure {@fig:Fig5S}I**).

![**Figure 5.** A, First two dimensions from UMAP of sample transcriptome data. Points are colored by `broad_histology` of the samples they represent. B, Heatmap of GSVA scores for Hallmark gene sets with significant differences, with samples ordered by `cancer_group`. C, Box plots of quanTIseq estimates of immune cell proportions in histologies with N > 15. D, Forest plot depicting the effect of immune cell proportion or _CD274_ expression on overall survival of medulloblastomas. Hazard ratio (HR) with 95% confidence intervals and p-values are listed and significant p-values are denoted with colored diamonds. Of note, the upper CI for macrophage M1 HR was too large to plot.](images/main/Figure5.png){#fig:Fig5}