...
The majority (N = 95) are medulloblastomas and span the spectrum of molecular subtypes: WNT, SHH, Group3, and Group 4 (see **Molecular Subtyping of CNS Tumors**) and their canonical mutations. 
...
We found _TP53_ (57%, 35/61) and _H3F3A_ (52%, 32/61) to be the most mutated and co-occurring genes (**Figure {@fig:Fig2}A**), followed by frequent mutations in _ATRX_ (30% 18/61). 
...
We observed 8% (16/195) of tumors to be _C11orf95::RELA_ fusion positive ependymomas and 12% (23/195) to be adamantinomatous craniopharayngiomas driven by mutations in _CTNNB1_. 
...
C, Diffuse astrocytic and oligodendrogliomal tumors (N = 61): diffuse midline gliomas (N = 34) and non-midline high-grade gliomas (N = 26), oligodendroglioma (N = 1); 
...
We observed _TP53_ to be the most frequently mutated gene across OpenPBTA tumors (8.4%, 56/666), significantly co-occurring with _H3F3A_ (35.2%, OR = 32, q = 8.46e-17), _ATRX_ (21.2%, OR = 20, q = 4.43e-8), _NF1_ (14.1%, OR = 8.62, q = 5.45e-5), and _EGFR_ (10%, OR = 18.2, q = 1.6e-4). 
Other canonical cancer driver genes _BRAF, H3F3A, CTNNB1, NF1, ATRX, FGFR1,_ and _PIK3CA_ were frequently mutated. 
Although LGG and embryonal tumors make up the majority of tumor types within the OpenPBTA, most of the significant gene interactions stem from HGATs (N = 847/872). At the broad histology level, _TP53_ and _PPM1D_ were mutually exclusive in HGATs (96.3%, OR = 0.188, p = 4.13e-2, q = 5.87e-2), _CTNNB1_ co-occurs with _TP53_ (35.7%, OR = 42.9%, q = 1.63e-3) and _DDX3X_ (31.3%, OR = 21.1, q = 4.46e-3) in embryonal tumors, and _FGFR1_ and _PIK3CA_ significantly co-occur in LGGs (33.3%, OR = 76.1, q = 3.26e-3), consistent with previous findings (@doi:10.1038/ng.2938).

Stub for **C** de novo mutational signatures. 

![**Figure 3. Mutational co-occurrence and signatures highlight key oncogenic drivers.**.
A, Barplot of occurrence and co-occurrence of nonsynonymous mutations for the 50 most commonly mutated genes across all tumor types (annotated from `cancer_group` if N >= 10 or `Other` if N < 10);
...
D, The number of SV breaks significantly correlate with CNV breaks (Adjusted R = 0.436, p = 1.08e-37) and as breakpoint density increases, the number of chromothripsis regions called increases (colored circles and **Figure {@fig:FigS3}B-C**). 
E, Chromothripsis frequency across pediatric brain tumors shown by `cancer_group` with N >= 3.
](https://github.com/AlexsLemonade/OpenPBTA-analysis/images/main/Figure3.png){#fig:Fig3}


### Genomic instability of pediatric brain tumors 

We generated high-confidence regions of amplification and deletion through consensus and focal copy number analysis (**STAR Methods**).
HGATs, followed by medulloblastomas, have the most genomically unstable genomes compared to copy number silent craniopharyngiomas (**Figure {@fig:FigS3}A**), consistent with their respective median TMBs and distribution (**Figure {@fig:FigS2}G**). 
The number of SV and CNV breakpoints were significantly correlated across tumors (Adjusted R = 0.436, p = 1.08e-37) (**Figure {@fig:Fig3}D**) and as expected, as breakpoint density increases, the number of chromothripsis regions called increases (**Figure {@fig:FigS3}B-C**).
**Figure {@fig:Fig3}E** shows the proportion of tumors per histology with chromothripsis events. Figure 3E shows the proportion of tumors per histology with chromothripsis events. 
We observed 41% (N = 19/46) of non-midline high-grade gliomas, 28.2% (N = 11/39) of DMGs, and >= 15% of ATRTs, medulloblastomas, ependymomas, meningiomas, germinomas, CNS embryonal tumors, glial-neuronal tumors, chordomas, ETMRs, metastatic secondary tumors, and sarcomas to have evidence of chromothripsis, highlighting the genomic instability and complexity of these tumors.

### Transcriptomic Landscape of Pediatric Brain Tumors

We performed UMAP (Uniform Manifold Approximation and Projection) unsupervised dimension reduction and geneset variant analysis (GSVA) for Hallmark cancer gene sets to validate that brain tumor histologies cluster based on tissue of origin (Figure 4A) and activation of underlying oncogenic pathways (Figure 4B).  

![**Figure 4. Brain tumors cluster by histology of origin and oncogenic pathway.**
A) First two dimensions from Uniform Manifold Approximation and Projection (UMAP) of transcriptome sample data, colored by `short_histology`.
