...
Estimating tumor fraction from RNA directly is challenging since methods focus on estimating proportion of immune cells and assuming the rest is tumor [@doi:10.1038/ncomms3612], which is not a fair assumption for many of these diagnoses.
We therefore used Theta2 (as described in the "Somatic Copy Number Variant Calling section" Methods section) to infer tumor purity of WGS samples and assumed that co-extracted RNA and DNA samples had the same tumor purity.
