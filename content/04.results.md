...
We used gene expression data sets from both poly-A and stranded RNA-Seq protocols to predict telomerase activities using EXTEND as shown in Supplementary Table {@tab:telomerase-activities}A , Table {@tab:telomerase-activities}B, Table {@tab:telomerase-activities}C and Table {@tab:telomerase-activities}D.
...
For both platforms, EXTEND scores were highly consistent between FPKM versus counts data as shown in Figure {@fig:comparison-plots}A and Figure {@fig:comparison-plots}B.
  
![Scatter plots comparing telomerase activities via Spearman correlation for counts with FPKM format.
A) poly-A platform .
B) stranded platform.
](https://github.com/AlexsLemonade/OpenPBTA-analysis/tree/master/analyses/telomerase-activity-prediction/plots/PTBA_GE_Score_AllScatter.pdf){@fig:comparison-plots}

We next examined telomerase activity across different brain tumor histological subtypes using data from stranded FPKM protocol. 
Across the short histological groups, we found germinomas had the highest EXTEND scores. 
However, teratoma, another type of germ cell tumor, had much lower scores as shown in Figure {@fig:distribution-plots}A.
Teratomas were previously reported to have negligible or low activities [@doi:10.1093/jnci/91.15.1321]. 
Benign neoplasia including dysplasia and hemangioblastomas had the lowest scores. 
Among common pediatric brain tumors, medulloblastomas had higher scores than high grade astrocytic tumors, ependymomas and low-grade astrocytoma. 
Across broad groups, benign tumors and precancerous lesions were among those with the lowest scores whereas tumors of pineal region and embryonal tumors have the highest scores as shown in Figure {@fig:distribution-plots}B. 
In case of medulloblastoma, high risk Group3 and Group4 subtypes had higher scores than WNT and SHH subtype as shown in Figure {@fig:subtype-comparison}. 
...
](https://github.com/AlexsLemonade/OpenPBTA-analysis/tree/master/analyses/telomerase-activity-prediction/plots/PBTA_StrandedHistology.pdf){@fig:distribution-plots}


![Box plot representing comparison of telomerase activities across molecular subtypes of medulloblastoma tumors. P-values have been calculated using series of two-sided t-tests and corrected via Bonferroni correction.
](https://github.com/AlexsLemonade/OpenPBTA-analysis/tree/master/analyses/telomerase-activity-prediction/plots/PBTA_MedulloSubtypes.pdf){@fig:subtype-comparison}
