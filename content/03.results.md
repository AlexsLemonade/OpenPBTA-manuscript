...
Stage 0 and/or 1 tumors characterized by low TMBs **Figure {@fig:S4}B** such as LGGs, gangliogliomas, craniopharyngiomas, DNETs, and schwannomas are expectedly dominated by Signature 1, which results from the normal process of spontaneous deamination of 5-methylcytosine.
...
Specifically, we annotated _TP53_ alterations as "activated" if samples harbored one of p.R273C or p.R248W mutations [@doi:10.1038/ng0593-42], or "lost" if the originating patient either had a Li Fraumeni Syndrome (LFS) predisposition diagnosis, the sample harbored a known hotspot mutation, or the sample contained two hits (e.g. both SNV and CNV), which would suggesting both alleles had been affected.
...
By contrast, the classifier showed somewhat lower accuracy on poly-A samples (AUROC = 0.62; **Figure {@fig:S5}A**).
...
We further compared the _TP53_ scores calculated by the classifier.
...
Although we were unable to detect canonical somatic _TP53_ mutations in two LFS patient tumors with low _TP53_ scores (BS_DEHJF4C7 with a score of 0.09 and BS_ZD5HN296 with a score of 0.28), we confirmed the LFS diagnoses from pathology reports and found each to have a pathogenic germline variant in _TP53_.
In addition, the tumor purity of these two LFS samples was low (16% and 37%, respectively) **TODO: Where are these numbers from? I don't see in the metadata file.**, suggesting the classifier may require a certain level of tumor purity to achieve good performance, as we expect _TP53_ to be intact in normal cells. 
...
We found aggressive tumors such as **TODO: Not all of these are explicitly laid out in 4D - rephrase based on 4D?** CNS lymphoma, ETMR, ATRT, DMG, and HGG had high EXTEND scores (**Figure {@fig:Fig4}D**), while benign lesions such as teratomas, dysplasias, and hemangioblastomas had the lowest scores (**Table S3**).
...
We further investigated the mutational signature profiles of the hypermutant (TMB > 10 Mut/Mb; N = 2) and ultra-hypermutant (TMB > 100 Mut/Mb; N = 4) tumors and/or derived cell lines, all of which were HGGs, from five patients in the OpenPBTA cohort (**Figure {@fig:Fig4}E**).
...
The MMR2 signature was present in tumors of four patients (PT_0SPKM4S8, PT_3CHB9PK5, PT_JNEV57VK, and PT_VTM2STE3) diagnosed with Lynch syndrome (**Table 2**), a genetic predisposition syndrome caused by a variant in a mismatch repair gene such as _PMS2_, _MLH1_, _MSH2_, _MSH6_, or others [@doi:10.3390/genes5030497].
Interestingly, while the cell line derived from patient PT_VTM2STE3's tumor at progression was not hypermutated (TMB = 5.74 Mut/Mb), it only showed exposure the MMR2 signature, suggesting selective pressure to maintain a mismatch repair (MMR) phenotype _in vitro_.
From patient PT_JNEV57VK, only one of the two cell lines derived from the progressive tumor was hypermuted (TMB = 35.9 Mut/Mb).
This hypermutated cell line had a strong exposure to signature 11, while this patient's non-hypermutated cell line showed a number of lesser exposures (1, 11, 18, 19, MMR2), highlighting the plasticity of mutational processes and the need to carefully genomically characterize and select models for preclinical studies based on research objectives.
...
Additionally, we found that all of these tumor or cell lines had dysfunctional _TP53_ (**Table 2**), consistent with a previous report showing _TP53_ dysregulation is a dependency of tumors with high genomic instability [@doi:10.1038/s43018-020-0027-5].
With one exception, hypermutant and ultra-hypermutant tumors had high  _TP53_ scores (> 0.5) and telomerase activity **Figure {@fig:Fig4}D**. **TODO: 1/3 of the telomerase activities are <0.5, so what counts as "high" here?**
...
Next, we asked whether transcriptomic classification of _TP53_ dysregulation and/or telomerase activity recapitulated the known prognostic influence of these oncogenic biomarkers.
...
Although higher _TP53_ scores, which would indicate _TP53_ gene or pathway dysregulation, were not a significant predictor of risk across the entire OpenPBTA cohort (**Table S4**), separate cox regressions constructed for individual cancer groups revealed significant survival risk for both DMGs (HR = 1.77e6, 95% CI = 1.98-1.57e12, p = 0.04) and ependymomas (HR = 1612, 95 % CI = 9.1-2.9e5, p = 0.005).
...
Interestingly, although H3 K28-mutant tumors have different biological drivers than do H3 wildtype tumors **TODO: Is there a reference for different biological drivers?**, they did not form distinct clusters. 
This pattern suggests these subtypes may be driven by common transcriptional programs, have other much stronger biological drivers such as their known distinct epigenetic drivers **TODO: this phrase confuses me because I don't see how _distinct_ epigenetic drivers would cause clustering together?** , or our sample size is too small to detect transcriptional differences.

We next performed gene set variant analysis (GSVA) for Hallmark cancer gene sets to demonstrate activation of underlying oncogenic pathways (**Figure {@fig:Fig5}B**, and we characterized the immune cell landscape across OpenPBTA tumors using the quanTIseq method which estimates absolute fractions of immune cell types (**Figure {@fig:Fig5}C** and **Figure {@fig:S6}E**).
These analyses recapitulated several previously-described trends.
...
We observed this trend in GSVA hallmark pathways (Bonferroni-corrected p < 0.05) (**Figure {@fig:Fig5}B**) and immune cell deconvolution, which showed that monocytes dominated the identified immune cells in these tumors (**Figure {@fig:Fig5}C**).
...
Surprisingly, the classically immunologically-cold HGG and DMG tumors [@doi:10.1186/s40478-018-0553-x; 10.1093/brain/awab155] contained higher overall fractions of immune cells, with monocytes, dendritic cells, and NK cells the most prevalent (**Figure {@fig:Fig5}C**).
This finding suggests that quanTIseq might actually be capturing microglia within these immune cell fractions.
While we did not detect notable prognostic effects of immune cell infiltration in HGG or DMG cancer groups, we did find that high levels of macrophage M1 and monocytes were associated with poorer overall survival (monocyte HR = 5.2e17, 95% CI = 1.2e5 - 2.3e30, p = 0.006) in medulloblastoma tumors (**Figure {@fig:Fig5}D**).
...
Consistent with a previous study [@doi:10.1080/2162402X.2018.1462430], we found higher expression of _CD274_ in the WNT subtype, which has the best prognosis of all medulloblastoma subgroups, compared to all other molecular subtypes (**Figure {@fig:Fig5}E**).
However, we also found that higher expression of _CD274_ was significantly, although marginally, associated with improved overall prognosis (HR = 0.00074, 95% CI = 3.0e-6 - 0.18, p = 0.01) for medulloblastoma samples (**Figure {@fig:Fig5}D**)
Finally, we asked whether any molecular subtypes might show an immunologically-hot phenotype, as roughly defined by a greater proportion of CD8+ to CD4+ T cells. **TODO: reference for this ratio?**
