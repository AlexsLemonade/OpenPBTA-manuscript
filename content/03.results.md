...
As new analytical products that we expected to be used across multiple analyses were produced (e.g., tumor mutation burden calculations) or issues with the data were identified, new releases were made available in a versioned manner.

A key feature of this project was the open contribution model used for both analyses (i.e., analytical code) and scientific manuscript writing. 
...
Additions to the manuscript often started with proposing an analysis in the analysis repository.
A potential contributor could propose an analysis by filing an issue on the GitHub analysis repository. 
Organizers for the project or other contributors with expertise had the opportunity to comment on the proposal (**Figure {@fig:Fig1}C**). 
...
All pull requests to the analysis repository underwent peer review by organizers or other contributors. 
Analytical code reviews conducted by peer reviewers checked for correctness of analyses (**Figure {@fig:Fig1}C-D**) and maintainability and readability of code and documentation. 

The nature of the project required additional steps beyond peer review of analytical code (**Figure {@fig:Fig1}D**).
Because multiple contributors were working on the code base and months would sometimes pass between updates to specific analysis modules, it was important to maintain a consistent software development environment to ensure that code could be revised. 
We leveraged Docker® and the Rocker project [@arxiv:1710.03675] to create a monolithic image that contained all the dependencies necessary for the analyses in the analysis repository. 
We used continuous integration (CI) to make sure that the image would build and that code would execute (**Figure {@fig:Fig1}D**). 
We also needed ensure that code did not cease to function over time, as data releases may introduce changes the order or number of samples. 
Using CI to run analytical code on a small subset of data used specifically for testing allowed us to check for any sensitivity to changes in the underlying data prior to code review (**Figure {@fig:Fig1}D**).

The Manubot-powered [@url:https://manubot.org/; @doi:10.1371/journal.pcbi.1007128] manuscript repository allowed us to follow a similar process for additions to this manuscript (**Figure {@fig:Fig1}C**). 
Contributors forked the manuscript repository and added proposed methods and results to their branch.
Contributors then filed pull requests to the main manuscript repository with their changes.
These pull requests underwent a review process that included peer review for clarity and correctness, agreement with interpretation, and spell checking via Manubot (**Figure {@fig:Fig1}C**).
