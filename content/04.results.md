...
We developed a method for generating consensus copy number alterated regions from MantaSV, CNVkit, and ControlFREEC (**STAR Methods**). 
...
We observed that HGATs, followed by medulloblastomas, have the most unstable genomes compared to copy number silent tumors such as craniopharyngiomas (**Figure S3A**) and this aligned with tumor mutational burden (TMB) (**Figure S2G**). 
...
We also found evidence of chromothripsis in over 15% of ATRTs, medulloblastomas, ependymomas, meningiomas, germinomas, CNS embryonal tumors, glial-neuronal tumors, chordomas, ETMRs, metastatic secondary tumors, and sarcomas to have evidence of chromothripsis, highlighting the genomic instability and complexity of pediatric brain tumors.

### Transcriptomic Landscape of Pediatric Brain Tumors

#### Histologic and oncogenic pathway clustering

We performed UMAP unsupervised dimension reduction to validate that brain tumors cluster by histology (**Figure {@fig:Fig4}A**) and geneset variant analysis (GSVA) for Hallmark cancer gene sets to demonstrate activation of underlying oncogenic pathways (**Figure {@fig:Fig4}B**).	
...
Additionally, _C11orf95::RELA_ fusion positive ependymomas and _BRAF_-driven low-grade tumors formed distinct clusters.
...
EXTEND scores calculated using FPKM or counts were tightly correlated (p < 2.2e-16) for each library type (**Figure {@fig:FigS4}E-F**) and significantly correlated with _TERT_ and _TERC_ expression (**Figure {@fig:FigS4}G-J**).  
We found aggressive tumors such as CNS lymphoma, ETMR, ATRT, DMG, and HGG had high EXTEND scores (**Figure {@fig:Fig4}C**), while benign lesions such as teratomas, dysplasias, and hemangioblastomas had the lowest scores (**Table S3**). 
...
Points are colored by `cancer_group` of the samples they represent.
B, Heatmap of GSVA scores for Hallmark gene sets with significant differences, with samples ordered by `cancer_group`.
C, EXTEND scores by cancer group (boxplots)](images/main/Figure4.png){#fig:Fig4}
